...
date: "2026-08-17"
published: true
description: "313 trials across sixteen models, one Rust scenario, the same eight engineering intents. Lift ranged from 0.00 to 0.89 depending only on which model read them — and the strongest model in the set was nowhere near the top."
tags:
  - GenAI
...
What's new is scale. Seven Claude models through Claude Code, five GPT models through Codex, and four local models through opencode against ollama — each on the driver that bills it correctly, so a big sweep runs against subscriptions and rate limits rather than metered tokens. Ten trials per arm throughout, with one exception I'll flag when I get to it: Qwen3.8's guided arm is still at three and filling in as I write.
...
| Qwen3.8 27B *(local, guided n=3)* | 0.086 | 0.636 | **0.551** | [0.326, 0.723] |
...
| Qwen3.6 27B *(local)* | 0.014 | 0.338 | **0.324** | [0.207, 0.440] |
| GPT-5.5 | 0.100 | 0.329 | **0.229** | [0.093, 0.356] |
| Haiku 4.5 | 0.043 | 0.229 | **0.186** | [0.074, 0.300] |
| GPT-5.6-luna | 0.186 | 0.357 | **0.171** | [0.024, 0.310] |
| Gemma4 31B *(local)* | 0.086 | 0.257 | **0.171** | [0.046, 0.293] |
| Qwen3.5 122B *(local)* | 0.114 | 0.171 | 0.057 | [-0.061, 0.175] |
| GPT-5.4 | 0.114 | 0.114 | 0.000 | [-0.110, 0.110] |

Bold means the interval excludes zero. Fourteen of sixteen do.

Fourteen of the sixteen models completed the task in every single trial, in both arms — 1.0 acceptance, no exceptions. Two didn't, and neither one is guidance breaking the software.

Qwen3.5 122B fails in both arms at about the same rate, 0.70 unguided and 0.80 guided. It cannot reliably build this crate, guidance or no guidance.
...
| Qwen3.5 122B | 450s | 435s | 1.0× | 0.057 |
...
Three of the four are now at ten trials a side, same as everything else. **Qwen3.8's guided arm is still at three and filling in as I write** — so treat that one row as a direction rather than a result.
