...
Notice how the 3.5-turbo model wanted to build a whole new class to hold structured input for the use case, but the 4.0 model generated more concise python code that captured the input data as an untyped dictionary.
...
In each iterative pass, the sample code reinforce the code style generated by the LLM. This reduces the amount of context we need to expend providing english-language description for the code style we want.

At the end, we are really stretching the 8KB context limit of GPT-4.

Now think about the couplings.
